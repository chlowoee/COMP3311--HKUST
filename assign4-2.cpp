...
void showTeach() {
	...
	// Add your code here
	SQLAllocStmt(hdbc, &hstmt);
	sprintf_s(query, "SELECT course.course_ID AS \"COURSE_ID\", course.course_name AS \"COURSE_NAME\", \
	offering.offering_no AS \"OFFERING_NO\", offering.classroom AS \"CLASSROOM\", offering.no_of_stds AS \
	\"NO_OF_STDS\" FROM course, offering, prof_teach \
	WHERE course.course_ID = offering.course_ID AND prof_teach.staff_ID =\'%i\' \
	AND prof_teach.offering_no=offering.offering_no", staff_id);

	SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	SQLBindCol(hstmt, 1, SQL_C_CHAR, course_ID, 8, &cbcourse_ID);
	SQLBindCol(hstmt, 2, SQL_C_CHAR, course_name, 80, &cbcourse_name);
...
void showTAs() {
	...
	char first_name[80];
	int phone;
	SQLINTEGER cbstudent_ID, cbfirst_name, cblast_name, cbphone;
	system("CLS");
	...
	// Add your code here
	SQLAllocStmt(hdbc, &hstmt);
	sprintf_s(query, "SELECT DISTINCT TA.student_ID AS \"STUDENT_ID\",TA.first_name AS \"FIRST_NAME\", \
	TA.last_name AS \"LAST_NAME\", TA.phone AS \"PHONE\" FROM TA, pref_ta, prof \
	WHERE pref_ta.staff_ID =\'%i\' \
	AND TA.student_id = pref_ta.student_id", staff_id);

	SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	SQLBindCol(hstmt, 1, SQL_INTEGER, &student_ID, 10, &cbstudent_ID);
	SQLBindCol(hstmt, 2, SQL_C_CHAR, first_name, 80, &cbfirst_name);
	SQLBindCol(hstmt, 3, SQL_C_CHAR, last_name, 80, &cblast_name);
...
void showPre() {
	...
	// Add your code here
	SQLAllocStmt(hdbc, &hstmt);
	sprintf_s(query, "SELECT prerequisite.main_course_ID AS \"MAIN_COURSE_ID\", \
	LISTAGG(prerequisite.prereq_course_ID,',') WITHIN GROUP(ORDER BY prerequisite.prereq_course_ID) \
	AS \"LISTAGG(PREREQ_COURSE_ID,',')\" FROM prerequisite GROUP BY prerequisite.main_course_ID");
	SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	printIntoRow(hstmt, 15);
	system("pause");
}
...
void showSuper() {
	...
	int student_ID;
	char full_name[161];
	SQLINTEGER cbstudent_ID, cbfull_name;
	system("CLS");
	...
	// Add your code here
	sprintf_s(query, "SELECT TA.student_ID AS \"STUDENT_ID\", TA.first_name ||','|| TA.last_name \
	AS \"FULLNAME\" FROM TA, supervise \
	WHERE supervise.student_ID = TA.student_ID AND supervise.staff_ID = \'%i\'", staff_id);

	SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
	SQLBindCol(hstmt, 1, SQL_INTEGER, &student_ID, 10, &cbstudent_ID);
	SQLBindCol(hstmt, 2, SQL_C_CHAR, full_name, 161, &cbfull_name);
	printIntoRow(hstmt, 15);
	system("pause");
}
...
void showSuperGroup() {
	...
	// Add your code here
	sprintf_s(query, "SELECT prof.staff_ID AS \"STAFF_ID\", prof.first_name ||' '|| prof.last_name AS \
	\"STAFF_FULL_NAME\", LISTAGG(supervise.student_ID || ':' || nvl(TA.first_name,' ')|| ' '|| nvl(TA.last_name,' '),',')\
	WITHIN GROUP(ORDER BY TA.student_ID) AS \"STUDENTS\" \
	FROM TA, SUPERVISE, PROF \
	WHERE supervise.staff_ID=prof.staff_ID AND supervise.student_ID=TA.student_ID \
...
void addPhone() {
	...
	// Add your code here
	SQLAllocStmt(hdbc, &hstmt);
	sprintf_s(query, "INSERT INTO prof_phone(staff_ID, phone_number) VALUES (\'%i\',\'%i\'); ", staff_id, newPhone);
	SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);
...
boolean ConnectDB() {
	...
	ret = SQLConnectA(hdbc, (SQLCHAR*)"comp3311.cse.ust.hk", SQL_NTS, (SQLCHAR*)"comp3311stu079", SQL_NTS, (SQLCHAR*)"lhcjbfkr", SQL_NTS);
	system("CLS"); // clear the screen
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO)
		return TRUE;
	return FALSE;
